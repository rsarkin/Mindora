...
export const PodDashboard: React.FC<{ podId?: string; onBack?: () => void }> = ({ podId: propPodId, onBack: propOnBack }) => {
    const { podId: paramPodId } = useParams<{ podId: string }>();
    const navigate = useNavigate();

    const podId = propPodId || paramPodId;
    const onBack = propOnBack || (() => navigate('../community'));
    const { user } = useAuth();
