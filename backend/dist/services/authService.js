"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.getSessionStatusService = exports.createAnonymousSessionService = exports.loginUser = exports.registerUser = void 0;
const bcryptjs_1 = __importDefault(require("bcryptjs"));
const jsonwebtoken_1 = __importDefault(require("jsonwebtoken"));
const User_1 = __importDefault(require("../models/User"));
const Therapist_1 = __importDefault(require("../models/Therapist"));
const AnonymousSession_1 = __importDefault(require("../models/AnonymousSession"));
const JWT_EXPIRES_IN = process.env.JWT_EXPIRES_IN || '7d';
const JWT_SECRET = process.env.JWT_SECRET;
var UserRole;
(function (UserRole) {
    UserRole["PATIENT"] = "PATIENT";
    UserRole["THERAPIST"] = "THERAPIST";
    UserRole["ADMIN"] = "ADMIN";
})(UserRole || (UserRole = {}));
const registerUser = async (data, files) => {
    const { name, email, password, role, specialization, specializations, bio, experienceYears, hourlyRate, licenseNumber, licenseState, phone, location } = data;
    const normalizedRole = role && role.toUpperCase() === 'THERAPIST' ? UserRole.THERAPIST : role && role.toUpperCase() === 'ADMIN' ? UserRole.ADMIN : UserRole.PATIENT;
    const existingUser = await User_1.default.findOne({ email });
    if (existingUser) {
        throw new Error('User already exists');
    }
    const passwordHash = await bcryptjs_1.default.hash(password, 10);
    // Process Multer files
    const avatarPath = files?.avatar?.[0]?.filename || undefined;
    const documentPaths = files?.documents?.map((f) => f.filename) || [];
    const user = await User_1.default.create({
        name,
        email,
        passwordHash,
        role: normalizedRole,
        accountStatus: normalizedRole === UserRole.THERAPIST ? 'PENDING_VERIFICATION' : 'ACTIVE',
        phone,
        location,
        avatar: avatarPath
    });
    if (normalizedRole === UserRole.THERAPIST) {
        let parsedSpecs = [];
        try {
            parsedSpecs = specializations ? JSON.parse(specializations) : (specialization ? [specialization] : []);
        }
        catch {
            parsedSpecs = typeof specializations === 'string' ? [specializations] : (specialization ? [specialization] : []);
        }
        await Therapist_1.default.create({
            userId: user._id,
            licenseNumber: licenseNumber || `TEMP_${Date.now()}`,
            licenseState: licenseState || 'NA',
            licenseExpiryDate: new Date(new Date().setFullYear(new Date().getFullYear() + 1)),
            specializations: parsedSpecs,
            bio: bio || '',
            experienceYears: experienceYears ? parseInt(experienceYears, 10) : 0,
            hourlyRateINR: hourlyRate ? parseFloat(hourlyRate) : 0.0,
            hourlyRateUSD: hourlyRate ? parseFloat(hourlyRate) / 80 : 0.0,
            verificationStatus: 'PENDING',
            verificationDocuments: documentPaths
        });
    }
    const token = jsonwebtoken_1.default.sign({ id: user._id, role: user.role }, JWT_SECRET, { expiresIn: JWT_EXPIRES_IN });
    return {
        token,
        user: { id: user._id, name: user.name, email: user.email, role: user.role.toLowerCase() }
    };
};
exports.registerUser = registerUser;
const loginUser = async (data) => {
    const { email, password } = data;
    const user = await User_1.default.findOne({ email });
    if (!user || !user.passwordHash) {
        const error = new Error('No account found with this email.');
        error.status = 404;
        throw error;
    }
    const isMatch = await bcryptjs_1.default.compare(password, user.passwordHash);
    if (!isMatch) {
        const error = new Error('Invalid email or password. Please try again.');
        error.status = 401;
        throw error;
    }
    if (user.role === 'THERAPIST' && user.accountStatus === 'PENDING_VERIFICATION') {
        const error = new Error('Your account is pending admin verification.');
        error.status = 403;
        throw error;
    }
    const token = jsonwebtoken_1.default.sign({ id: user._id, role: user.role }, JWT_SECRET, { expiresIn: JWT_EXPIRES_IN });
    return {
        token,
        user: { id: user._id, name: user.name, email: user.email, role: user.role.toLowerCase() }
    };
};
exports.loginUser = loginUser;
const createAnonymousSessionService = async (data) => {
    const { deviceFingerprint, initialEmotion, initialMessage } = data;
    if (!deviceFingerprint || typeof deviceFingerprint !== 'string') {
        const error = new Error('deviceFingerprint is required and must be a string');
        error.status = 400;
        throw error;
    }
    if (deviceFingerprint.length < 10 || deviceFingerprint.length > 500) {
        const error = new Error('deviceFingerprint must be between 10 and 500 characters');
        error.status = 400;
        throw error;
    }
    const existingSession = await AnonymousSession_1.default.findOne({ deviceFingerprint });
    if (existingSession) {
        if (existingSession.status === 'EXPIRED') {
            const error = new Error('This anonymous session has expired. Please start a new session.');
            error.status = 410;
            error.codeName = 'Session Expired';
            throw error;
        }
        if (existingSession.status === 'CONVERTED_TO_USER') {
            const error = new Error('This device is already associated with a registered account. Please log in.');
            error.status = 409;
            error.codeName = 'Session Already Converted';
            error.userId = existingSession.convertedUserId;
            throw error;
        }
        const token = jsonwebtoken_1.default.sign({ sessionId: existingSession._id, type: 'anonymous', deviceFingerprint: existingSession.deviceFingerprint }, JWT_SECRET, { expiresIn: JWT_EXPIRES_IN });
        return {
            status: 200,
            data: { token, sessionId: existingSession._id, messageCount: existingSession.messageCount, maxFreeMessages: existingSession.maxFreeMessages, isExisting: true }
        };
    }
    const temporaryName = `Guest_${Math.random().toString(36).substring(2, 8)}`;
    const newSession = await AnonymousSession_1.default.create({
        deviceFingerprint, temporaryName, initialMessage: initialMessage || null, detectedEmotion: initialEmotion || null, messageCount: 0, maxFreeMessages: 5, status: 'ACTIVE',
    });
    const token = jsonwebtoken_1.default.sign({ sessionId: newSession._id, type: 'anonymous', deviceFingerprint: newSession.deviceFingerprint }, JWT_SECRET, { expiresIn: JWT_EXPIRES_IN });
    return {
        status: 201,
        data: { token, sessionId: newSession._id, temporaryName: newSession.temporaryName, messageCount: newSession.messageCount, maxFreeMessages: newSession.maxFreeMessages, isExisting: false }
    };
};
exports.createAnonymousSessionService = createAnonymousSessionService;
const getSessionStatusService = async (sessionId) => {
    if (!sessionId) {
        const error = new Error('sessionId is required');
        error.status = 400;
        throw error;
    }
    const session = await AnonymousSession_1.default.findById(sessionId);
    if (!session) {
        const error = new Error('Session not found');
        error.status = 404;
        throw error;
    }
    return {
        sessionId: session._id,
        status: session.status,
        messageCount: session.messageCount,
        maxFreeMessages: session.maxFreeMessages,
        hasReachedLimit: session.messageCount >= session.maxFreeMessages,
        expiresAt: session.expiresAt,
        convertedUserId: session.convertedUserId
    };
};
exports.getSessionStatusService = getSessionStatusService;
//# sourceMappingURL=authService.js.map